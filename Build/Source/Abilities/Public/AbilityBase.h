...
protected:
	...
	bool SetState(EAbilityState NewState, FStructContainer Container,
		EAbilityTransitionFlag Flags = EAbilityTransitionFlag::None);
	...
	void ServerSetState(
		FAbilityStateTransition Transition, const FStructContainer& Container, uint32 RequestedStateId);
	...
	void MCSetState(
		FAbilityStateTransition Transition, const FStructContainer& Container, uint32 ServerStateId);
...
private:
	...
	{
		LastRequestedStateId =
			(LastRequestedStateId > CurrentStateId ? LastRequestedStateId : CurrentStateId) + 1;
...
public:
	...
	FStructContainer& GetCurrentContainer()
	{
		return CurrentContainer;
	}
	const FStructContainer& GetCurrentContainer() const
	{
		return CurrentContainer;
	}
};

inline bool UAbilityBase::CheckTransition(
	FAbilityStateTransition Transition, const FStructContainer& Container)
{
...
{
	return Owner != nullptr && State != EAbilityState::BeforeBeginPlay;
}
