...
void UAbilitiesComponent::TickComponent(
...
bool UAbilitiesComponent::ReplicateSubobjects(
	UActorChannel* Channel, class FOutBunch* Bunch, FReplicationFlags* RepFlags)
{
	...
	for (int32 I = 0; I < AllAbilities.Num(); ++I)
	{
		...
		else
		{
			...
			AllAbilities.RemoveAtSwap(I, 1, EAllowShrinking::No);
			--I;
		}
	}
...
void UAbilitiesComponent::MCAddRemainingCooldowns_Implementation(
	const TArray<UClass*>& Classes, float Duration)
{
...
void UAbilitiesComponent::GetBuffsOfClass(TSubclassOf<UBuff> Class, TSet<FBuffCount>& OutBuffs) const
{
	...
	if (const auto* Container = BuffsByClass.Find({Class.Get()}))
	{
		...
		for (auto* Buff : Container->Buffs)
		{
			...
			if (ensureMsgf(
					BuffCount, TEXT("Container had a buff that is not present in the global list of buffs.")))
			{
...
void UAbilitiesComponent::ClientOnBuffsChanged_Implementation(
	const TArray<FBuffCount>& ModifiedBuffs, EBuffOperation Change)
{
...
void UAbilitiesComponent::MCOnBuffsChanged_Implementation(
	const TArray<FBuffCount>& ModifiedBuffs, EBuffOperation Change)
{
	if (!HasAuthority())	// Ignore server
	{
