...
bool FStructContainer::NetSerialize(FArchive& Ar, UPackageMap* Map, bool& bOutSuccess)
{
	...
	if (Ar.IsLoading())
	{
		...
		Structs.SetNum(Num, EAllowShrinking::Yes);
		Data.Empty(DataSize);
	}
...
bool UStructContainerLibrary::Generic_AddStruct(
	FStructContainer& Container, FProperty* Property, void* DataPtr, bool bReplace)
{
	auto* StructProp = CastField<FStructProperty>(Property);
	MakeSureMsg(StructProp, TEXT("UStructContainerLibrary::AddStruct: Only structs can be added. Any other "
								 "type is not supported.")) false;

	return Container.Add(StructProp, DataPtr, bReplace);
}

bool UStructContainerLibrary::Generic_GetStruct(
	FStructContainer& Container, FProperty* Property, void* DataPtr)
{
	auto* StructProp = CastField<FStructProperty>(Property);
	MakeSureMsg(StructProp, TEXT("UStructContainerLibrary::GetStruct: Only structs can be contained. Any "
								 "other type is not supported.")) false;
